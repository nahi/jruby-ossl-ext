...
public class Lookup {
    ...
    public int loadCertificateFile(String file, int type) throws Exception {
        BouncyCastleASN1FormatHandler formatHandler = new BouncyCastleASN1FormatHandler();
        if (file == null) {
            return 1;
        }
        int count = 0;
        ...
        try {
            ...
            if (type == X509Utils.X509_FILETYPE_PEM) {
                ...
                for (;;) {
                    x = formatHandler.readX509Aux(reader, null);
                    if (null == x) {
                        break;
                    }
    ...
    public int loadCRLFile(String file, int type) throws Exception {
        ...
        try {
            ...
            if (type == X509Utils.X509_FILETYPE_PEM) {
                reader = new BufferedReader(new InputStreamReader(in));
                for (;;) {
                    x = OpenSSLReal.getFormatHandler().readX509CRL(reader, null);
                    if (null == x) {
                        break;
                    }
    ...
    public int loadCertificateOrCRLFile(String file, int type) throws Exception {
        ...
        try {
            ...
            reader = new BufferedReader(new InputStreamReader(in));
            for (;;) {
                Object v = OpenSSLReal.getFormatHandler().readPEM(reader, null);
                if (null == v) {
                    break;
                }
